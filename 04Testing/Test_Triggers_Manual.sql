...
DECLARE @AuctionId BIGINT = (SELECT TOP 1 AuctionId FROM auction.Auction WHERE StatusCode = 'ACTIVE');
DECLARE @CurrentPrice DECIMAL(38,18) = (SELECT CurrentPriceETH FROM auction.Auction WHERE AuctionId = @AuctionId);
DECLARE @Bidder1 BIGINT = (SELECT TOP 1 UserId FROM core.UserRole WHERE RoleId = 4 ORDER BY UserId);
DECLARE @Bidder2 BIGINT = (SELECT UserId FROM core.UserRole WHERE RoleId = 4 ORDER BY UserId OFFSET 1 ROWS FETCH NEXT 1 ROWS ONLY);
DECLARE @Bidder3 BIGINT = (SELECT UserId FROM core.UserRole WHERE RoleId = 4 ORDER BY UserId OFFSET 2 ROWS FETCH NEXT 1 ROWS ONLY);

PRINT 'Precio actual: ' + CAST(@CurrentPrice AS VARCHAR) + ' ETH';
PRINT '';

-- Primera oferta
PRINT '--- Oferta 1 ---';
INSERT INTO auction.Bid(AuctionId, BidderId, AmountETH)
VALUES(@AuctionId, @Bidder1, @CurrentPrice + 0.1);
PRINT 'Oferente 1 ofrece: ' + CAST(@CurrentPrice + 0.1 AS VARCHAR) + ' ETH';
PRINT '';

-- Segunda oferta (mayor)
PRINT '--- Oferta 2 ---';
INSERT INTO auction.Bid(AuctionId, BidderId, AmountETH)
VALUES(@AuctionId, @Bidder2, @CurrentPrice + 0.25);
PRINT 'Oferente 2 ofrece: ' + CAST(@CurrentPrice + 0.25 AS VARCHAR) + ' ETH';
PRINT '';

-- Tercera oferta (mayor aún)
PRINT '--- Oferta 3 ---';
INSERT INTO auction.Bid(AuctionId, BidderId, AmountETH)
VALUES(@AuctionId, @Bidder3, @CurrentPrice + 0.5);
PRINT 'Oferente 3 ofrece: ' + CAST(@CurrentPrice + 0.5 AS VARCHAR) + ' ETH';
PRINT '';

-- Verificar resultados
PRINT '--- Estado de la Subasta ---';
SELECT 
    a.AuctionId,
    a.StartingPriceETH,
    a.CurrentPriceETH,
...
SELECT 
    ...
    b.PlacedAtUtc,
    CASE 
        WHEN b.BidderId = (SELECT CurrentLeaderId FROM auction.Auction WHERE AuctionId = @AuctionId)
        THEN '👑 LÍDER'
        ELSE ''
    END as Estado
FROM auction.Bid b
...
DECLARE @AuctionId2 BIGINT = (SELECT TOP 1 AuctionId FROM auction.Auction WHERE StatusCode = 'ACTIVE');
DECLARE @CurrentPrice2 DECIMAL(38,18) = (SELECT CurrentPriceETH FROM auction.Auction WHERE AuctionId = @AuctionId2);
DECLARE @TestBidder BIGINT = (SELECT TOP 1 UserId FROM core.UserRole WHERE RoleId = 4 ORDER BY UserId);

PRINT 'Precio actual: ' + CAST(@CurrentPrice2 AS VARCHAR) + ' ETH';
PRINT 'Intentando ofertar: ' + CAST(@CurrentPrice2 - 0.1 AS VARCHAR) + ' ETH (MENOR)';
PRINT '';

-- Intentar oferta inválida
INSERT INTO auction.Bid(AuctionId, BidderId, AmountETH)
VALUES(@AuctionId2, @TestBidder, @CurrentPrice2 - 0.1);
...
FROM auction.Bid b
JOIN core.[User] u ON u.UserId = b.BidderId
ORDER BY b.AuctionId, b.AmountETH DESC;
PRINT '';
